...
from tools.game_state import death_screen_features as new_death_screen_features  # noqa: E402
...
def old_death_screen_features(image: Image.Image) -> dict[str, float]:
    ...
    return {
        "bottom_green_ratio": region_ratio(
            (0.10, 0.65, 0.90, 0.95), is_bright_green
        ),
        "progress_green_ratio": region_ratio(
            (0.18, 0.18, 0.82, 0.30), is_bright_green
        ),
        "center_dark_ratio": region_ratio(
            (0.10, 0.10, 0.90, 0.90), is_nearly_black
        ),
    }
